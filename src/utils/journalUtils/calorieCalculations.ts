...
export const calculateCaloriesBurned = ({
  ...
  const metValue =
    MET_VALUES[activityType][
      intensity.toLowerCase() as "low" | "medium" | "high"
    ];
  ...
  if (userProfile?.age && userProfile?.gender && userProfile?.height) {
    if (userProfile.gender === "male") {
      bmr =
        10 * userWeight + 6.25 * userProfile.height - 5 * userProfile.age + 5;
    } else {
      bmr =
        10 * userWeight + 6.25 * userProfile.height - 5 * userProfile.age - 161;
    }
